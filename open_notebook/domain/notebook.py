...
async def resolve_notebook_scope(notebook_ids: List[str]) -> List[str]:
    ...
    is indistinguishable from "no matches". Existence is checked with one
    query for the whole list. Returns the ids unchanged (empty list = whole
    knowledge base). Raises InvalidInputError for malformed ids, NotFoundError
    for unknown notebooks.
    """
    if not notebook_ids:
        return []

    # Only notebook ids are accepted: a source or note id would fail the
    # typed record<notebook> parameter inside SurrealDB, and "notebook:"
    # with an empty key would blow up in RecordID.parse.
    ...
    # RecordID objects are unhashable and never equal request strings —
