...
compose() {
  APP_IMAGE="datafabricx/open-notebook-commercial:$TAG" DATA_DIR="$RC_DATA" \
...
case "$1" in
  ...
  up)
    ...
    docker pull "datafabricx/open-notebook-commercial:$TAG" || \
      echo "WARNING: could not pull datafabricx/open-notebook-commercial:$TAG — using the local image if present."
    ...
    echo "RC stack up — image datafabricx/open-notebook-commercial:$TAG"
