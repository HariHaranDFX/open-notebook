...
export function ShareSheet({
  ...
  const resourceLabel = t(
    resourceType === 'notebook' ? 'common.notebook' : 'common.source',
  );
  ...
  const isMutating =
    createGrant.isPending || updateGrant.isPending || deleteGrant.isPending;
    ...
    <>
      <Sheet open={open} onOpenChange={handleSheetOpenChange}>
        <SheetContent
          ...
          className="flex w-[min(440px,calc(100vw-24px))] flex-col gap-0 rounded-l-none p-0"
        >
          <SheetHeader className="gap-0 border-b border-border px-5 py-3">
          ...
          <div className="flex-1 space-y-6 overflow-y-auto px-5 py-4">
            {accessSummary && (
              <section
                ...
                className="space-y-2"
              >
                <Label id="share-sheet-access-heading">
                  {t('sharing.yourAccess')}
                </Label>
            ...
            <section
              aria-labelledby="share-sheet-grants-heading"
              className="space-y-2"
            >
              <Label id="share-sheet-grants-heading">
                {t('sharing.grants')}
              </Label>
              {isLoading ? (
                ...
                <ul className="space-y-2">
                  {grants.map((grant) => {
                    const isUpdatingThisGrant =
                      updateGrant.isPending &&
                      updateGrant.variables?.grantId === grant.id;
                    const updateFailed =
                      updateGrant.isError &&
                      updateGrant.variables?.grantId === grant.id;
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent side="top">{t('sharing.revoke')}</TooltipContent>
                          </Tooltip>
                        </div>
                        {updateFailed && (
                          <p
            ...
            >
              <Label id="share-sheet-add-heading">
                {t('sharing.principalType')}
              </Label>
              <div
                className="flex gap-2"
                data-testid="share-sheet-principal-row"
              >
                {showGroups && (
                  <Select
                    value={principalType}
                  ...
                  >
                    <SelectTrigger
                      ...
                      aria-label={t('sharing.principalType')}
                    >
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="flex gap-2">
                <Select
                  value={role}
                  onValueChange={(v) => setRole(v as GrantRole)}
                >
                  <SelectTrigger
                    className="w-32"
                    aria-label={t('sharing.roleLabel')}
          ...
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t('sharing.revokeConfirmTitle', {
                ...
                principal:
                  grantPendingRevoke?.principal_label ||
                  grantPendingRevoke?.principal_id ||
                  '',
              })}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t('sharing.revokeConfirmDescription', {
                role: grantPendingRevoke
                  ? t(`sharing.${grantPendingRevoke.role}`)
                  : '',
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          {deleteGrant.isError &&
            deleteGrant.variables === grantPendingRevoke?.id && (
              <p
                role="alert"
                aria-live="assertive"
