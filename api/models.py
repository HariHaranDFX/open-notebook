...
class NotebookLibraryPageResponse(BaseModel):
    """One page of the Notebooks library — keyset paginated.

    ``next_cursor`` is an opaque, backend-issued token; callers echo it back
    unchanged to fetch the next page. It is ``None`` when no further page
    exists. See ``api/pagination.py``.
    """
